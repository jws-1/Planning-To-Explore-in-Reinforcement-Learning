            ...
            while not done:
                ...
                if isinstance(action, MetaAction):
                    if action == MetaAction.INCREASE_REWARD:
                        ...
                        self.model.update_reward(state, target_action, target_state, self.model.get_reward(state, target_action, target_state)+1.0)
                    elif action == MetaAction.INCREASE_TRANSITION_PROBABILITY:
                        print(state, action, target_action, target_state)
                        self.model.update_transition(state, target_action, target_state, 1.0)
                    self.meta_sas[state][target_action][action] = True
                else:
                    print(state, action)
