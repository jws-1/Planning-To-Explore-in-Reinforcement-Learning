...
"""
So, steps
    1. Choose a candidate action
        Decide how to choose (visitation, time since last visitation, largest mean, largest uncertainty, entropy)
    2. Hypothesise change to the model that would most benefit (i.e., increase the probability of a high reward by taking an action)
    3. Take action, perform model-free learning
"""
...
class MetaUCB():
    ...
    def choose_arm(self, config):
        return np.argmin(self.avg_reward)
        # n = np.sum(self.N_a)
        # ucb_values = self.avg_reward +  np.sqrt(np.log(n) / (self.N_a + 1e-5))
    ...
    def learn(self, config):
        ...
        for i in range(config.max_actions):
            if i < config.planning_steps:
                pass
                # arm = np.argmin(self.avg_reward)
            else:
